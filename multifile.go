...
type multiFileLogWriter struct {
	writers       [LevelTrace + 1 + 1]*fileLogWriter // the last one for fullLogWriter
	fullLogWriter *fileLogWriter
	Separate      []string `json:"separate"`
}

var levelNames = [...]string{"emergency", "alert", "critical", "error", "warning", "notice", "info", "debug", "trace"}
...
func (f *multiFileLogWriter) Init(config string) error {
	...
	for i := LevelEmergency; i < LevelTrace+1; i++ {
